...
/* VARIABLES HOLDING DATA.  As declared in dft-data.h. */
...
static double *read_targets (numin_source *, int, dft_spec *);
static double *read_inputs  (numin_source *, int *, dft_spec *);


/* FREE SPACE OCCUPIED BY DATA.  Also useful as a way to reset when the
   model specification changes. */

void dft_data_free (void)
{
  if (train_inputs!=0)
  { free(train_inputs);
    train_inputs = 0;
...
/* READ TRAINING AND/OR TEST DATA.  Either or both of these data sets are 
   ...
   specifications are consistent with the model specifications. */
...
)
{
  ...
  { 
    ...
    train_inputs = read_inputs (&ns, &N_train, dft);

    numin_spec (&ns, data_spec->train_targets, data_spec->N_targets);
    train_targets = read_targets (&ns, N_train, dft);
  }
